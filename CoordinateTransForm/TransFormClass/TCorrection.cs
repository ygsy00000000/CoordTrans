...
{
    public class TCorrection : TCoordinate
    {
        ...
        /// 构造函数
        /// </summary>
        public TCorrection(EnumTransFormType type, int iScale,
        ...
        public CoordinatePoint Compute(CoordinatePoint p)
        {
            ...
            try
            {
                ...
                {
                    //// 经纬度转平面坐标
                    if (_type == EnumTransFormType.BLHtoXYH || _type == EnumTransFormType.BLHtoBLH)
                    {
                        TClass = new GaussTrans(EnumTransFormType.BLHtoXYH, EnumCoordinateFormat.Du, _earth_old, null, _dOldCentralMeridian, _dOldAddY);
                        p = TClass.Compute(p);
                    }
                    //// 新坐标系下的xy坐标         可能逻辑不对需要修改
                    NewPnt.XorLat = p.XorLat + _dB;
                    NewPnt.YorLon = p.YorLon + _dL;
                    //// 高斯反算转换，平面坐标转经纬度
                ...
                else
                {
                    ...
                    //// 度转秒
                    double Lats = TransFormMethod.DegreeToSecond(p.XorLat);
                    double Lons = TransFormMethod.DegreeToSecond(p.YorLon);
